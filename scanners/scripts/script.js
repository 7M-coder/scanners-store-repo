...
function phoneValidate() {
    ...
    if(phoneInput.search(/^9665+\d{8}/) != 0 && phoneInput.length > 0) {
        ...
        errors.innerHTML = '<p class="error">' + 'phone number should always start with 9665, and only contain digits.' + '</p>'
        return true;
    } 
...
function purchaseValidate() {
    ...
    if(checkFname || checkLname || checkPhone || checkEmail || checkCity || checkAddress || checkZip) {
        // if there are errors prevent submitting
        return false;
    }
